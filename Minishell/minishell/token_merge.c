...
/*   Updated: 2025/06/11 21:09:38 by oltolmac         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*m_handle_quote_exp(t_mini *mini, t_cmd *cmd, char *str, int start)
{
	...
	{
		if (str[cmd->x] == '$' && str[cmd->x + 1] != '='
			&& !is_space(str[cmd->x + 1]) && !is_dquote(str[cmd->x + 1]))
		{
			if (start == cmd->x)
...
char	*m_handle_dol(t_mini *mini, t_cmd *cmd, char *res, char *str)
{
	...
	tmp = NULL;
	mini->free = res;
	if (is_space(str[cmd->x + 1]) || is_quote(str[cmd->x + 1])
		|| str[cmd->x + 1] == '='
		|| !str[cmd->x + 1] || is_arrow(str[cmd->x + 1]))
