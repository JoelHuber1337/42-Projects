...
void	InternCheck()
{
	...
	stest = peter.makeForm("ShrubberyCreationForm", "default");
	rtest = peter.makeForm("RobotomyRequestForm", "default");
	ptest = peter.makeForm("PresidentialPardonForm", "default");
	wrong = peter.makeForm("wrong", "wrong");
	...
	stest->beSigned(correct);
	stest->execute(correct);
	rtest->beSigned(correct);
	rtest->execute(correct);
	ptest->beSigned(correct);
	ptest->execute(correct);
	delete stest;
	delete rtest;
	delete ptest;
}
